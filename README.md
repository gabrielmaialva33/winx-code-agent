...
and background-command metadata never feed orchestration decisions. Daemon protocol `1.4` transports that same typed
state end to end, so marker-looking text cannot manufacture or suppress a polling loop. A `ReadFiles` batch with any
...
not combine the protocol-level `task` object with `is_background=true`. Task results retain the same structured envelope,
are kept for the negotiated bounded TTL, and may be fetched repeatedly until expiry.
...
`typed_action_result`. An older durable guardian remains inspectable, but `BashCommand` fails closed until that session is
...
| `WINX_USAGE_LOG`                                           | Optional path for non-blocking JSONL `winx::usage` events. Contains tool/action, principal, scoped thread, hashed request/session correlation, client/protocol, outcome, result status, duration, and response size—never command text, file contents, or credentials. On Unix every initial/rotated file is opened `0600` (`O_NOFOLLOW`), and newly created log directories use `0700`. With `WINX_SANDBOX=1`, place it under the workspace, `/tmp`, or a `WINX_SANDBOX_RW_PATHS` root. |
