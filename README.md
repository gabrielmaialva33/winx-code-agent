...
| `EditFiles`       | Creates, changes, or undoes one or many files. One edit call may contain up to 100 unique targets and validates the full batch before writing. Each entry selects an explicit mode: `search_replace` for small exact edits, `line_patch` with a `ReadFiles` revision and visible coordinates, `replace` for new files or intentional whole-file rewrites, or `undo` with the exact returned `undo_id`. Optional `verify_command` runs once after commit; a failed check never repeats the edit. |
