...
pub fn read_file_optimized(path: &Path, max_file_size: u64) -> Result<Vec<u8>> {
    ...
    if file_size < DIRECT_READ_THRESHOLD {
        ...
        debug!(
            "Using segmented memory-mapped read for file: {}",
            path.display()
        );
        read_segmented_mmap(&file, file_size, path)
    } else {
        debug!(
            "Using streaming read for extremely large file: {}",
            path.display()
        );
...
fn read_direct(file: &File, file_size: u64, path: &Path) -> Result<Vec<u8>> {
    ...
    if file_size < 1_000_000 {
        ...
        let mut reader = BufReader::with_capacity(min(file_size as usize, 64 * 1024), file);
    ...
    let mut buffer = Vec::with_capacity(file_size as usize);
    let mut reader = BufReader::with_capacity(262_144, file); // 256KB buffer
...
fn read_mmap(file: &File, path: &Path) -> Result<Vec<u8>> {
    ...
    if mmap.len() > 10_000_000 {
        ...
        debug!(
            "Using parallel processing for large mmap file: {}",
            path.display()
        );
...
pub fn read_file_segment(
    ...
    if length < DIRECT_READ_THRESHOLD {
        ...
        debug!(
            "Using memory-mapped read for file segment: {}",
            path.display()
        );
