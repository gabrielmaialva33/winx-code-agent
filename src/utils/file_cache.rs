...
impl FileCache {
    ...
    pub fn read_file(&self, path: &Path) -> Result<Vec<u8>> {
                ...
                if let Ok(metadata) = fs::metadata(path) {
                    if metadata.modified()? == entry.last_modified {
                        ...
                        if let Some(content) = &entry.content {
                            ...
                            // Update access time in a separate scope
                            {
                                // Explicitly drop the read lock
                                drop(entries);
                                if let Ok(mut entries_write) = self.entries.write() {
                                    if let Some(entry) = entries_write.get_mut(path) {
                                        entry.touch();
                                    }
                                }
                            }
        ...
        if metadata.len() <= MAX_CACHED_FILE_SIZE {
            self.update_cache_entry(path, &content, &metadata)?;
        } else {
    ...
    pub fn record_read_range(&self, path: &Path, start: usize, end: usize) -> Result<()> {
        if let Ok(mut entries) = self.entries.write() {
            if let Some(entry) = entries.get_mut(path) {
                ...
                // File not in cache yet, read it first
                drop(entries);
                self.read_file(path)?;
