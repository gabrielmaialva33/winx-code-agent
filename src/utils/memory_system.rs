...
impl MemorySystem {
    ...
    fn ensure_memory_dir(&self) -> Result<()> {
        if !self.memory_dir.exists() {
            ...
            fs::create_dir_all(&self.memory_dir)
                .with_context(|| format!("Failed to create memory directory: {:?}", self.memory_dir))?;
        }
        Ok(())
    }
    ...
    pub fn format_memory(&self, task_memory: &ContextSave, relevant_files: &str) -> String {
        ...
        let quoted_globs: Vec<String> = task_memory.relevant_file_globs
            .iter()
    ...
    pub fn delete_memory(&self, task_id: &str) -> Result<()> {
        ...
        if state_file_path.exists() {
            fs::remove_file(&state_file_path)
                .with_context(|| format!("Failed to remove bash state file: {:?}", state_file_path))?;
...
mod tests {
    ...
    fn test_shell_unquoting() {
        ...
        assert_eq!(shell_unquote("\"path\\\"with\\\"quotes\""), Some("path\"with\"quotes".to_string()));
