...
use crate::state::persistence::{
    delete_bash_state as delete_state_file,
    load_bash_state as load_state_file,
    save_bash_state as save_state_file,
    BashStateSnapshot,
};
use crate::state::pty::PtyShell;
use crate::state::terminal::{
    incremental_text, TerminalEmulator, TerminalOutputDiff,
    DEFAULT_MAX_SCREEN_LINES, MAX_OUTPUT_SIZE as TERMINAL_MAX_OUTPUT_SIZE,
};
use crate::types::{
...
impl FileWhitelistData {
    ...
    pub fn get_unread_ranges(&self) -> Vec<(usize, usize)> {
        if self.total_lines == 0 { return vec![]; }
        let mut lines_read = std::collections::HashSet::new();
        for (start, end) in &self.line_ranges_read {
            for line in *start..=*end {
                lines_read.insert(line);
        ...
        for i in 1..=self.total_lines {
            if !lines_read.contains(&i) {
                if start_range.is_none() { start_range = Some(i); }
            } else if let Some(start) = start_range {
                unread.push((start, i - 1));
                start_range = None;
            }
        }
        if let Some(start) = start_range { unread.push((start, self.total_lines)); }
        unread
    }
    ...
    pub fn get_read_error_message(&self, file_path: &Path) -> String {
        format!("File {} needs more reading. Coverage: {:.1}%", file_path.display(), self.get_percentage_read())
    }
...
fn contains_wcgw_prompt(text: &str) -> bool { PROMPT_REGEX.is_match(text) }
...
impl InteractiveBash {
    pub fn is_alive(&mut self) -> bool { matches!(self.process.try_wait(), Ok(None)) }

    pub fn reinit(&mut self) -> Result<()> {
        let mut cmd = Command::new("bash");
        if self.restricted_mode { cmd.arg("-r"); }
        let mut process = cmd.env("PAGER", "cat").env("GIT_PAGER", "cat")
            .env("PROMPT_COMMAND", WCGW_PROMPT_COMMAND).env("TERM", "xterm-256color")
            .current_dir(&self.initial_dir).stdin(Stdio::piped()).stdout(Stdio::piped()).stderr(Stdio::piped())
            .spawn()?;
    ...
    pub fn ensure_alive(&mut self) -> Result<()> {
        if !self.is_alive() { self.reinit()?; }
        Ok(())
    }

    pub fn new(initial_dir: &Path, restricted_mode: bool) -> Result<Self> {
        let mut cmd = Command::new("bash");
        if restricted_mode { cmd.arg("-r"); }
        let mut process = cmd.env("PAGER", "cat").env("GIT_PAGER", "cat")
            .env("PROMPT_COMMAND", WCGW_PROMPT_COMMAND).env("TERM", "xterm-256color")
            .current_dir(initial_dir).stdin(Stdio::piped()).stdout(Stdio::piped()).stderr(Stdio::piped())
            .spawn()?;
        let mut stdin = process.stdin.take().ok_or_else(|| anyhow!("No stdin"))?;
        writeln!(stdin, "{BASH_PROMPT_STATEMENT}")?;
        stdin.flush()?;
        process.stdin = Some(stdin);
        Ok(Self {
            process, last_command: String::new(), last_output: String::new(),
            output_buffer: String::new(), command_state: CommandState::Idle,
            max_output_size: MAX_OUTPUT_SIZE, output_truncated: false,
            output_chunks: Vec::new(), initial_dir: initial_dir.to_path_buf(), restricted_mode,
        })
    }
    ...
    pub fn send_command(&mut self, command: &str) -> Result<()> {
        ...
        self.command_state = CommandState::Running { start_time: std::time::SystemTime::now(), command: command.to_string() };
        Ok(())
    }
    ...
    pub fn read_output(&mut self, timeout_secs: f32) -> Result<(String, bool)> {
            ...
            if let Some(stdout) = self.process.stdout.as_mut() {
                if let Ok(n) = stdout.read(&mut buf) {
                    if n > 0 {
                        ...
                        if contains_wcgw_prompt(&full_output) { complete = true; break; }
                    }
                }
            }
        ...
        if complete { self.command_state = CommandState::Idle; }
...
impl BashState {
    pub fn new() -> Self {
        ...
        Self {
            cwd: cwd.clone(), workspace_root: cwd, current_thread_id: generate_thread_id(),
            mode: Modes::Wcgw,
            bash_command_mode: BashCommandMode { bash_mode: BashMode::NormalMode, allowed_commands: AllowedCommands::All("all".to_string()) },
            file_edit_mode: FileEditMode { allowed_globs: AllowedGlobs::All("all".to_string()) },
            write_if_empty_mode: WriteIfEmptyMode { allowed_globs: AllowedGlobs::All("all".to_string()) },
            whitelist_for_overwrite: HashMap::new(), terminal_state: TerminalState::new(),
            interactive_bash: Arc::new(Mutex::new(None)), pty_shell: Arc::new(Mutex::new(None)),
            initialized: false,
        }
    }

    pub fn init_interactive_bash(&mut self) -> Result<()> {
        let bash = InteractiveBash::new(&self.cwd, self.bash_command_mode.bash_mode == BashMode::RestrictedMode)?;
    ...
    pub fn save_state_to_disk(&self) -> Result<()> {
        let snapshot = BashStateSnapshot::from_state(
            &self.cwd.to_string_lossy(), &self.workspace_root.to_string_lossy(),
            &self.mode, &self.bash_command_mode, &self.file_edit_mode,
            &self.write_if_empty_mode, &self.whitelist_for_overwrite, &self.current_thread_id,
        );
