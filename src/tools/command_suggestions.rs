...
fn get_command_explanation(command: &str) -> Cow<'static, str> {
    // Extract the base command (first word)
    let base_command = command.split_whitespace().next().unwrap_or(command);

    // Common command explanations (cached to avoid allocations)
    lazy_static::lazy_static! {
        static ref EXPLANATIONS: HashMap<&'static str, &'static str> = [
    ...
    if base_command == "git" {
        ...
        if parts.len() > 1 {
            ...
            lazy_static::lazy_static! {
                static ref GIT_EXPLANATIONS: HashMap<&'static str, &'static str> = [
            ...
            return GIT_EXPLANATIONS
                ...
                .unwrap_or(Cow::Owned("Git operation".to_string()));
        }
    }
    ...
    if base_command == "npm" {
        ...
        if parts.len() > 1 {
            ...
            lazy_static::lazy_static! {
                static ref NPM_EXPLANATIONS: HashMap<&'static str, &'static str> = [
            ...
            return NPM_EXPLANATIONS
                .get(npm_subcommand)
                .map(|&s| Cow::Borrowed(s))
                .unwrap_or(Cow::Owned("NPM operation".to_string()));
        }
    }
