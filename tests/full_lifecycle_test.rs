...
async fn create_initialized_state(temp_dir: &TempDir) -> Result<Arc<Mutex<Option<BashState>>>> {
    let bash_state_arc: Arc<Mutex<Option<BashState>>> = Arc::new(Mutex::new(None));
    let workspace_path = std::fs::canonicalize(temp_dir.path())?.to_string_lossy().to_string();
...
async fn test_full_lifecycle_workflow() -> Result<()> {
    ...
    timeout(TEST_TIMEOUT, async {
        ...
        let output =
            winx_code_agent::tools::bash_command::handle_tool_call(&state, mkdir_cmd).await?;
        ...
        let ls_output =
            winx_code_agent::tools::bash_command::handle_tool_call(&state, ls_cmd).await?;
        ...
        let read_output =
            winx_code_agent::tools::read_files::handle_tool_call(&state, read_cmd.clone()).await?;
...
>>>>>>> REPLACE"#
        ...
        let grep_output =
            winx_code_agent::tools::bash_command::handle_tool_call(&state, grep_cmd).await?;
        ...
        assert!(
            save_output.contains("test-save")
                || save_output.contains(".txt")
                || save_output.to_lowercase().contains("saved"),
            "Context check failed: {save_output}"
        );
    ...
    .map_err(|_| WinxError::CommandTimeout {
        command: "full lifecycle test".to_string(),
